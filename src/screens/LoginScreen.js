...
export default function LoginScreen({ onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isRegister) {
        ...
        data = await customerLogin({ mobile: mobile.trim(), ...credential });
      }
      onLogin(data);
    } catch (e) {
      ...
      } else if (!isRegister && /unauthorized|invalid/i.test(String(msg))) {
        setError(`No account found for that mobile. Tap "Create account" below to sign up.`);
      } else {
        setError(msg);
      }
    } finally {
  ...
  const ctaLabel = isRegister ? 'Create account' : (usingOtp ? 'Verify OTP' : 'Log in');
        ...
        >
          ...
          >
            ...
            <Text style={{ fontSize: 22, fontWeight: '800', color: '#0F172A', letterSpacing: -0.4 }}>
              {isRegister ? 'Create account' : 'Login or signup'}
            </Text>
            <Text style={{ fontSize: 13, color: MUTED, marginTop: 4, lineHeight: 19 }}>
              {isRegister
                ? 'Sign up in seconds to book trusted repairs nearby.'
            ...
            <FieldGroup label="Mobile number">
              <View style={fieldRow}>
                ...
                >
                  <Text style={{ fontSize: 15, color: '#0F172A', fontWeight: '600' }}>+91</Text>
                </View>
                <Input
            ...
            <FieldGroup label={usingOtp ? 'Enter OTP' : 'Password'}>
              ...
              {usingOtp ? (
                <Text style={hint}>Dev OTP: 123456 (real SMS OTP coming later)</Text>
              ) : null}
            </FieldGroup>
